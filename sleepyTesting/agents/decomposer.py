...
Breaks down high-level UI tasks into atomic operations using LLM-based task
analysis. Supports both Chinese and English task descriptions and handles
multi-device operations across Android, iOS, and web platforms.
"""
import logging
...
class TaskDecomposer:
    ...
    This class uses an LLM to analyze natural language task descriptions and
    break them down into atomic UI operations. It supports both Chinese and
    English input and can handle multi-device operations across Android, iOS,
    and web platforms.
    """
    ...
    def __init__(self, llm_provider: Optional[ILLMProvider] = None):
        ...
        Args:
            llm_provider: Optional ILLMProvider instance. If not provided,
    ...
    async def decompose_task(self, task_description: str) -> List[UIStep]:
        """
        Break down high-level task descriptions into specific UI steps.

        Uses LLM to analyze tasks and generate atomic UI operations. Supports
        both Chinese and English input and handles multi-device operations.
    ...
    def validate_steps(
        ...
        Example:
            validate_steps([
                ...
                UIStep(action="send_message", platform="android",
                      device_id="A123")
            ]) -> True
        """
        if not steps:
