...
DEVICE_IDENTIFIER: Optional[str] = os.getenv("SLEEPYTESTING_DEVICE_ID")  # Legacy support

# Whitelisted device IDs (optional)
ALLOWED_ANDROID_DEVICES: List[str] = [
    d.strip() for d in os.getenv("SLEEPYTESTING_ALLOWED_ANDROID_DEVICES", "").split(",")
    if d.strip()
]
ALLOWED_IOS_DEVICES: List[str] = [
    d.strip() for d in os.getenv("SLEEPYTESTING_ALLOWED_IOS_DEVICES", "").split(",")
    if d.strip()
]
...
def validate_device_id(device_id: Optional[str], platform: str) -> Optional[str]:
    ...
    if allowed_devices and device_id not in allowed_devices:
        raise ValueError(
            f"Device ID '{device_id}' not in allowed list for {platform}: {allowed_devices}"
        )
